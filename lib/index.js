...
module.exports = function createDataset(rawDataset, filledDataset, callback) {
  var dataset;
  if(!callback) {
    dataset = {};
    callback = filledDataset;
  }
  else {
    ...
    module.exports.config[model].generator(rawDataset[key], function(err, result) {
      ...
      for(var subkey in result) {
        if(result[subkey] instanceof Function) {
          result[subkey] = result[subkey](dataset);
        }
      }
      dataset[key] = result;
      cb(null);
  ...
  async.waterfall([
    ...
    function buildAutoBuildList(buildList, cb) {
      var asyncBuildList = {};
      Object.keys(buildList).forEach(function(model) {
        Object.keys(buildList[model]).forEach(function(key) {
          asyncBuildList[key] = [];
          // push the dependencies of the object
          if(module.exports.config[model].dependencies) {
            asyncBuildList[key] = asyncBuildList[key].concat(module.exports.config[model].dependencies);
          }
          // push the function for async.auto
          asyncBuildList[key].push(function(callback) {
            generateObject(model, key, callback);
          });
        });
      });
